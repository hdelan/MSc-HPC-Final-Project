...
int main(void)
{
  ...
  if (krylov_dim > n) krylov_dim = n-1;
  ...
  gettimeofday(&m_d, NULL);
  double gpu_time_lanczos {(m_d.tv_sec - s_d.tv_sec + (m_d.tv_usec - s_d.tv_usec) / 1000000.0)};
  ...
  double gpu_time_whole {gpu_time_lanczos + (e_d.tv_sec - m_d.tv_sec + (e_d.tv_usec - m_d.tv_usec) / 1000000.0)};
