...
void lanczosDecomp<T>::cu_decompose()
{
  ...
  auto global_memory_used { (n+1+2*A.edge_count)*sizeof(unsigned)+(n+2*krylov_dim-1+num_blocks)*sizeof(T)};
  ...
  {
    // v = A*Q(:,j)
    cu_spMV1<T, unsigned><<<blocks, threads>>>(IA_d, JA_d, n, Q_d_ptr[k], v_d); 

    // alpha = v*Q(:,j)
    if (num_blocks==1) { 
      cu_dot_prod<T, BLOCKSIZE><<<1, threads, 0, stream[0]>>>(v_d, Q_d_ptr[k], n, &alpha_d[k]);
    } else {
      cu_dot_prod<T, BLOCKSIZE><<<num_blocks/2, threads,0,stream[0]>>>(v_d, Q_d_ptr[k], n, tmp_d);
      cu_reduce<T, BLOCKSIZE><<<1, threads, 0, stream[0]>>>(tmp_d, num_blocks, &alpha_d[k]);
    }
    ...
    if (k < krylov_dim - 1)
    {
      // beta[j] = norm(v)
      if (num_blocks==1) {
        cu_norm_sq_sqrt<T, BLOCKSIZE><<<1, threads, 0, stream[0]>>>(v_d, n, &beta_d[k]);
      } else {
        cu_norm_sq<T, BLOCKSIZE><<<num_blocks/2, threads, 0, stream[0]>>>(v_d, n, tmp_d);
        cu_reduce_sqrt<T,BLOCKSIZE><<<1, threads, 0, stream[0]>>>(tmp_d, num_blocks, &beta_d[k]);
      }
      // Q(:,j) = v/beta
