...
__global__ void cu_spMV1(U *const IA, U *const JA, const U n, T *const x, T *ans)
{
...
__global__ void cu_spMV2(U *const IA /*row_offset*/, U *const JA /* col_idx*/, U *const blockrows, const U n, T *const x, T *ans)
{
  ...
  // Load row entries into shared memory
  ...
  // Reduce the rows with a single thread
...
/* --------------------------------------------------------------------------*/
/**
* \brief:        The child kernel for dynamically parallel kernel      
 *
 * \param:       JA
 * \param:       nnz
 * \param:       x
